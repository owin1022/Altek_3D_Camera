...
namespace librealsense
{
    ...
    public:
        ...
        std::vector<tagged_profile> get_profiles_tags() const override
        {
            ...
            if (usb_spec >= platform::usb3_type || usb_spec == platform::usb_undefined)
            {
                ...
                tags.push_back({ RS2_STREAM_COLOR, 0, 1280, 720, RS2_FORMAT_RGB8, 30, profile_tag::PROFILE_TAG_SUPERSET | profile_tag::PROFILE_TAG_DEFAULT });

            }
            else
            {
                tags.push_back({ RS2_STREAM_DEPTH, -1, 640, 360, RS2_FORMAT_Z16, 15, profile_tag::PROFILE_TAG_SUPERSET | profile_tag::PROFILE_TAG_DEFAULT });
                //tags.push_back({ RS2_STREAM_INFRARED, 1, 640, 480, RS2_FORMAT_Y8, 15, profile_tag::PROFILE_TAG_SUPERSET | profile_tag::PROFILE_TAG_DEFAULT });
                //tags.push_back({ RS2_STREAM_INFRARED, 2, 640, 480, RS2_FORMAT_Y8, 15, profile_tag::PROFILE_TAG_SUPERSET });
                tags.push_back({ RS2_STREAM_COLOR, 0, 640, 360, RS2_FORMAT_RGB8, 15, profile_tag::PROFILE_TAG_SUPERSET | profile_tag::PROFILE_TAG_DEFAULT });
            }
            return tags;
    ...
    // for al3d  AL3Di_PID
    class al3Di_device : public ds5_active,
        public ds5_color,
        public ds5_advanced_mode_base,
        public ds5_motion,
        public firmware_logger_device
    {
    public:
        al3Di_device(std::shared_ptr<context> ctx,
            const platform::backend_device_group& group,
                     bool register_device_notifications)
            : device(ctx, group, register_device_notifications),
              ds5_device(ctx, group),
    ...
    std::shared_ptr<matcher> al3Di_device::create_matcher(const frame_holder& frame) const
    {
        std::vector<stream_interface*> streams = { _depth_stream.get() , _left_ir_stream.get() , _right_ir_stream.get(), _color_stream.get() };
        // TODO - A proper matcher for High-FPS sensor is required
        std::vector<stream_interface*> mm_streams = { _accel_stream.get(), _gyro_stream.get()};
