...
namespace librealsense
{
	...
	void al32_converter::process_function(byte * const dest[], const byte * source, int width, int height, int actual_size, int input_size)
	{
		auto depth_size = width*height*2;
		auto ir_size =  width*height;
		...
		std::memcpy(dest[0], source, depth_size);
		std::memcpy(dest[1], source + depth_size, ir_size);
		std::memcpy(dest[2], source + (depth_size + ir_size), ir_size);
	}
}
