...
namespace librealsense
{
    ...
    interleaved_functional_processing_block::interleaved_functional_processing_block(const char* name,
		rs2_format source_format,
		rs2_format depth_target_format,
		rs2_stream depth_target_stream,
		rs2_extension depth_extension_type,
		int depth_idx,		  
		rs2_format left_target_format,
		rs2_stream left_target_stream,
		rs2_extension left_extension_type,
        ...
        :   processing_block(name),
            _source_format(source_format),
			_depth_target_format(depth_target_format),
			_depth_target_stream(depth_target_stream),
			_depth_extension_type(depth_extension_type),
			_depth_target_profile_idx(depth_idx),
            _left_target_format(left_target_format),
            _left_target_stream(left_target_stream),
            _left_extension_type(left_extension_type),
            _left_target_profile_idx(left_idx),
            ...
            _right_target_profile_idx(right_idx)
    {
        configure_processing_callbackEx();
    }

	// Ken++  Z32
    void interleaved_functional_processing_block::configure_processing_callbackEx()
    {
        // define and set the frame processing callback
        auto process_callback = [&](frame_holder frame, synthetic_source_interface* source)
        {
            ...
            {
                _source_stream_profile = profile;
				_depth_target_stream_profile = profile->clone();
                _right_target_stream_profile = profile->clone();
                _left_target_stream_profile = profile->clone();

				_depth_target_bpp = get_image_bpp(_depth_target_format) / 8;
                _left_target_bpp = get_image_bpp(_left_target_format) / 8;
                _right_target_bpp = get_image_bpp(_right_target_format) / 8;

                _depth_target_stream_profile->set_format(_depth_target_format);
                _depth_target_stream_profile->set_stream_type(_depth_target_stream);
                _depth_target_stream_profile->set_stream_index(_depth_target_profile_idx);
                _depth_target_stream_profile->set_unique_id(_depth_target_profile_idx);
            ...
            frame_holder dp, lf, rf;

		    dp = source->allocate_video_frame(_depth_target_stream_profile, frame, _depth_target_bpp,
                w, h, w * _depth_target_bpp, _depth_extension_type);
            ...
            planes[0] = (byte*)dp.frame->get_frame_data();
            planes[1] = (byte*)lf.frame->get_frame_data();
			planes[2] = (byte*)rf.frame->get_frame_data();

            process_function(planes, (const byte*)frame->get_frame_data(), w, h, 0, 0);
			
			source->frame_ready(std::move(dp));
            source->frame_ready(std::move(lf));
            source->frame_ready(std::move(rf));
        };
